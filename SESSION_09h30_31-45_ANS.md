# SESSION 09:30 (31-45 ANS)
## IA, cybersécurité & expérience métier — travailler plus vite, protéger sa famille, monétiser son savoir
...
- **Les bons réflexes :**
  ...
  2. **Ne scannez pas aveuglément :** Évitez de scanner des QR codes douteux ou collés par-dessus d'autres affichages.
...
- **Les Dangers Majeurs :**
  ...
  - **Failles de sécurité ignorées :** En copiant-collant aveuglément le code généré, les utilisateurs laissent des failles de sécurité majeures dans leurs applications, facilement exploitables par les pirates.
- **La Règle d'Or (Sécurisation des clés) :** 
  - Stockez toujours vos clés d'API et mots de passe dans un fichier `.env`.
  - **Crucial :** Ajoutez impérativement ce fichier `.env` à votre fichier `.gitignore` pour éviter qu'il ne soit publié accidentellement sur des plateformes comme GitHub.
...
- **Procédure en direct (2 minutes) :**
  ...
  3. Activer la validation en deux étapes (**2FA / Double Authentification**).
